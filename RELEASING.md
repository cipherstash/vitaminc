...
- Version bumps in `Cargo.toml` (all 13 crates are bumped together)
...
All 13 workspace crates are in a single `version_group`. This means every release bumps all crates to the same version, regardless of which crates had changes. This keeps version alignment simple.
