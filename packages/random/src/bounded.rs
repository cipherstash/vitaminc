...
///
/// [`next_below(n)`](BoundedRng::next_below) is the primitive: a value
/// uniform in `0..n`, the half-open bound every index-shaped use wants (`n`
/// items, pick one) and the one `std` and `rand` ranges use. It is
/// implemented for `u32` and for a [`Protected<u32>`] bound, and is also
/// reachable as [`SafeRand::next_below`] without importing this trait.
///
/// [`next_bounded(max)`](BoundedRng::next_bounded) is the older
/// **inclusive** form, `0..=max`. It is kept for compatibility and
...
/// doc always said, for every `max` up to and including `u32::MAX`.
///
/// Both forms make exactly one 64-bit draw per call, reduced with Lemire's
/// multiply-high method: no rejection loop and no branch on the value
/// drawn, so the number of draws does not depend on the values drawn. The
...
pub trait BoundedRng<T> {
    ...
    ///
    /// Deprecated in favour of [`next_below`](BoundedRng::next_below); see
    /// the trait docs for why. The equivalent call is `next_below(max + 1)`
    /// (for `max == u32::MAX` that is the whole word: use
...
/// A uniformly distributed value in `0..range`, for `range >= 1`.
///
...
/// [`below_u64`] at `u32` width: uniform in `0..range`.
pub(crate) fn below_u32<R: CryptoRng>(rng: &mut R, range: u32) -> u32 {
    below_u64(rng, u64::from(range)) as u32
}

/// The inclusive form, uniform in `0..=max`, for every `max` including
